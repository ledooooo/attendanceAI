...
export default function CalculatorsMenu() {
  ...
  const calculators = [
    // --- باطنة وعامة ---
    { id: 'frax', category: 'general', title: 'مقياس FRAX للهشاشة', description: 'حساب احتمالية الكسور خلال 10 سنوات (WHO).', icon: Bone, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'ottawa', category: 'general', title: 'قواعد Ottawa للأشعة', description: 'تحديد الحاجة لأشعة الكاحل والركبة بعد الإصابة.', icon: Footprints, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'centor', category: 'general', title: 'مقياس Centor', description: 'تقييم احتمالية التهاب الحلق البكتيري.', icon: Stethoscope, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'curb65', category: 'general', title: 'مقياس CURB-65', description: 'تقييم خطورة الالتهاب الرئوي المكتسب.', icon: Activity, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'gfr', category: 'general', title: 'وظائف الكلى (CrCl)', description: 'تقدير معدل التصفية لتعديل جرعات الأدوية.', icon: Activity, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'bmi', category: 'general', title: 'مؤشر كتلة الجسم', description: 'تقييم حالة السمنة أو النحافة والوزن المثالي.', icon: Scale, color: 'bg-blue-50 text-blue-600' },
    ...
    { id: 'heart-rate', category: 'general', title: 'نبضات القلب', description: 'النطاقات المثالية للنبض أثناء الرياضة.', icon: Heart, color: 'bg-rose-50 text-rose-600' },
    { id: 'lab-values', category: 'general', title: 'دليل التحاليل', description: 'النسب الطبيعية لأشهر التحاليل الطبية.', icon: Dna, color: 'bg-indigo-50 text-indigo-600' },
    { id: 'screening', category: 'general', title: 'الفحص الشامل', description: 'الفحوصات الدورية المطلوبة حسب السن.', icon: Stethoscope, color: 'bg-emerald-50 text-emerald-600' },
    ...
    // --- أطفال وطوارئ ---
    { id: 'pecarn', category: 'peds', title: 'قاعدة PECARN', description: 'تقييم إصابات الرأس للأطفال (أشعة أم ملاحظة).', icon: AlertCircle, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    ...
    // --- عيادة الأسنان ---
    { id: 'cambra', category: 'dental', title: 'مخاطر التسوس', description: 'نظام CAMBRA لتصنيف مخاطر التسوس (ADA).', icon: ShieldAlert, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'periodontal', category: 'dental', title: 'أمراض اللثة', description: 'نظام AAP لتصنيف شدة أمراض اللثة.', icon: Activity, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'dental-age', category: 'dental', title: 'العمر السني', description: 'تقدير عمر الطفل بناءً على بزوغ الأسنان.', icon: Baby, color: 'bg-sky-50 text-sky-600' },

    // --- الصيدلية ---
    { id: 'beers', category: 'pharmacy', title: 'معايير Beers', description: 'الأدوية التي يجب تجنبها لكبار السن (AGS).', icon: AlertCircle, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'child-pugh', category: 'pharmacy', title: 'مقياس Child-Pugh', description: 'تقييم شدة تليف الكبد لتعديل الجرعات.', icon: Activity, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'ivf', category: 'pharmacy', title: 'المحاليل الوريدية', description: 'حساب سرعة التنقيط واحتياجات السوائل.', icon: Droplets, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    { id: 'fib4', category: 'pharmacy', title: 'مؤشر تليف الكبد', description: 'تقييم درجة التليف (FIB-4) بدون خزعة.', icon: Activity, color: 'bg-red-50 text-red-600', isDoctorOnly: true },
    ...
    { id: 'visual-conv', category: 'all', title: 'تحويل حدة الإبصار', description: 'التحويل بين مقاييس Snellen و LogMAR.', icon: Eye, color: 'bg-indigo-50 text-indigo-600' },
  ];
