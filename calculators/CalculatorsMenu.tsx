...
import { 
  Scale, Baby, Calendar, FileText, Brain, Heart, 
...
import CVDRiskCalculator from './CVDRiskCalculator'; // الحاسبة الجديدة
...
export default function CalculatorsMenu() {
  ...
  const calculators = [
    ...
    { id: 'osteoporosis', category: 'risks', title: 'هشاشة العظام', description: 'فحص سريع لعوامل الخطر لضعف العظام.', icon: Bone, color: 'bg-slate-100 text-slate-700 border-slate-200' },
  ...
  return (
    <div className="font-sans animate-in fade-in duration-500 pb-10">
      ...
      {/* Header */}
      <div className="bg-white rounded-3xl p-5 md:p-6 shadow-sm border border-gray-100 mb-4 flex flex-col md:flex-row items-center justify-between gap-4 relative overflow-hidden">
      ...
      <div className="flex overflow-x-auto gap-2 pb-3 mb-4 custom-scrollbar">
        {categories.map(cat => (
          <button
            key={cat.id}
      ...
      {filteredCalculators.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5 md:gap-4">
          {filteredCalculators.map((calc, idx) => {
            ...
            return (
              ...
              >
                <div className="bg-white p-3 md:p-5 rounded-2xl md:rounded-[2rem] border shadow-sm hover:shadow-md hover:border-blue-100 hover:-translate-y-1 transition-all duration-300 h-full flex flex-col border-transparent">
                  ...
                  <div className="flex items-center justify-between text-[9px] md:text-[10px] font-black mt-auto pt-2.5 md:pt-3 border-t border-gray-50 text-gray-400 group-hover:text-blue-600 transition-colors">
        ...
        <div className="text-center py-16 bg-white rounded-[2rem] border border-gray-100 mt-4">
          <Search className="w-10 h-10 text-gray-200 mx-auto mb-3"/>
          <p className="text-gray-500 font-bold text-xs md:text-sm">لا توجد حاسبات مطابقة في هذا القسم</p>
          <button onClick={() => {setSearchTerm(''); setActiveCategory('all');}} className="text-blue-600 font-bold text-[10px] md:text-xs mt-3 bg-blue-50 px-4 py-2 rounded-xl hover:bg-blue-100 transition-colors">عرض الكل</button>
        </div>
      )}
