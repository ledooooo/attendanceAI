...
export default function NotificationBell({ onNavigate }: Props) {
  ...
  useEffect(() => {
    const fetchUserDate = async () => {
      ...
      try {
        ...
        if (supData && !supError) {
          setUserCreatedAt(supData.created_at);
        }
      } catch (error) {
  ...
  const handlePushActivate = async () => {
    ...
    if (success) {
      alert('تم ربط جهازك بنجاح! ✅');
    } else {
      alert('فشل التسجيل. تأكد من السماح بالإشعارات في المتصفح.');
    }
  };
    ...
    <div className="relative">
      ...
      >
        <Bell className={`w-5 h-5 md:w-6 md:h-6 ${
          unreadCount > 0 ? 'text-orange-600 animate-pulse' : 'text-gray-600'
        }`} />
        {unreadCount > 0 && (
      ...
      {isOpen && (
        <>
          ...
          <div className={`
            ...
            bg-white rounded-[2rem] shadow-2xl border border-gray-100 z-[100] overflow-hidden 
          ...
          `}>
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              ...
              >
                <Smartphone size={12}/> 
                {permission === 'granted' ? 'مفعل ✓' : 'تفعيل'}
              </button>
              <button 
                onClick={() => setIsOpen(false)} 
                className="md:hidden p-1 bg-white rounded-full text-gray-400 hover:text-gray-600"
              >
                <X size={16}/>
              </button>
            </div>
                ...
                filteredNotifications.map((notif) => (
                  <div 
                    ...
                    className={`
                      ...
                      ${!notif.is_read ? 'bg-orange-50/40 border-l-4 border-l-orange-400' : ''}
                    `}
                  >
      ...
      {isOpen && (
        <div 
          className="hidden md:block fixed inset-0 z-[80]" 
          onClick={() => setIsOpen(false)}
        />
      )}
    </div>
