...
const GAME_TYPES = [
    ...
    { key: 'beast',      label: '🦁 ليفل الوحش',    icon: '🦁',   desc: '+5000 نقطة — رهان',    color: 'from-red-600 to-orange-700',   minPlayers: 2, maxPlayers: 4  },
...
// ─── Share Room (native share + clipboard fallback) ───────────────────────────
async function shareRoom(matchId: string, gameLabel: string) {
    const link = getRoomLink(matchId);
    const text = `تحداني في لعبة ${gameLabel}! انضم الآن 🎮`;
    if (navigator.share) {
        try { await navigator.share({ title: 'تحدي الصالة', text, url: link }); return; } catch { /* fallback */ }
    }
    try {
...
interface OnlineEmployee { employee_id: string; name: string; specialty: string | null; photo_url: string | null; last_seen: string | null; }

function AdminOnlinePanel({ adminEmployee, currentMatchId, gameLabel }: { adminEmployee: Employee; currentMatchId: string | null; gameLabel: string; }) {
    ...
    const fetchOnline = async () => {
        ...
        const { data } = await supabase.from('employees').select('employee_id, name, specialty, photo_url, last_seen')
            .gte('last_seen', cutoff).neq('employee_id', String(adminEmployee.employee_id)).neq('role', 'admin').order('last_seen', { ascending: false });
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel('online_presence_admin')
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'employees', filter: 'role=neq.admin' }, () => fetchOnline())
            .subscribe();
    ...
    const sendInvite = async (emp: OnlineEmployee) => {
        ...
        await supabase.from('notifications').insert({
            ...
            message: `تم دعوتك للانضمام لغرفة ${gameLabel}! ${link}`,
            type: 'competition', is_read: false,
        });
        await sendPushToUser(String(emp.employee_id), `🎮 دعوة لعبة من ${adminEmployee.name?.split(' ')[0] || 'المدير'}`, `انضم لغرفة ${gameLabel} الآن!`, link);
            ...
            <button onClick={() => setCollapsed(p => !p)}
                ...
                <div className="flex items-center gap-2">
                    <div className="relative">
                        ...
                        {online.length > 0 && <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-yellow-400 text-gray-900 text-[9px] font-black rounded-full flex items-center justify-center">{online.length}</span>}
                    </div>
                    <span className="font-black text-sm">الموظفون أونلاين الآن{online.length === 0 && <span className="text-green-200 font-bold text-xs mr-1">(لا أحد)</span>}</span>
                </div>
                <div className="flex items-center gap-2">
                    {currentMatchId && online.length > 0 && !collapsed && (
                        <button onClick={e => { e.stopPropagation(); sendToAll(); }} disabled={sendingAll}
                            ...
                            {sendingAll ? <Loader2 className="w-3 h-3 animate-spin"/> : <Send className="w-3 h-3"/>} دعوة الكل
                        </button>
                    )}
                ...
                <div className="p-3">
                    {online.length === 0 ? (
                        ...
                        <div className="space-y-2 max-h-56 overflow-y-auto">
                            {online.map(emp => {
                                const isSending = sending[emp.employee_id], isSent = sent[emp.employee_id];
                                const style = AVATAR_STYLES[styleIdx(emp.employee_id)];
                                return (
                                    <div key={emp.employee_id} className="flex items-center gap-2.5 p-2 rounded-xl hover:bg-gray-50">
                                        <div className="relative flex-shrink-0">
                                            {emp.photo_url ? <img src={emp.photo_url} alt={emp.name} className={`w-9 h-9 rounded-full object-cover ring-2 ${style.ring}`}/>
                                                : <div className={`w-9 h-9 rounded-full bg-gradient-to-br ${style.bg} flex items-center justify-center ring-2 ${style.ring}`}><span className="text-white font-black text-xs">{initials(emp.name)}</span></div>}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                {emp.specialty && <span className="text-[10px] text-gray-400 font-bold truncate max-w-[90px]">{emp.specialty}</span>}
                                                <span className="text-[9px] text-green-500 font-bold">{timeAgo(emp.last_seen)}</span>
                                            </div>
                                        </div>
                                        <button onClick={() => sendInvite(emp)} disabled={isSending || isSent || !currentMatchId}
                                            className={`flex-shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-xl font-black text-[11px] transition-all active:scale-95 ${
                                                isSent ? 'bg-green-100 text-green-700 border border-green-200' :
                                                !currentMatchId ? 'bg-gray-100 text-gray-400 cursor-not-allowed' :
                                                'bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm'}`}>
                                            {isSending ? <Loader2 className="w-3 h-3 animate-spin"/> : isSent ? <><CheckCircle2 className="w-3 h-3"/> تم</> : <><Send className="w-3 h-3"/> دعوة</>}
                                        </button>
                                    </div>
                                );
                            })}
                    ...
                    {!currentMatchId && <p className="text-[10px] text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5 mt-2 font-bold text-center">أنشئ غرفة أولاً لتتمكن من إرسال الدعوات</p>}
                </div>
            )}
        </div>
    );
...
export default function LiveGamesArena({ employee, onClose, initialRoomId }: LiveGamesArenaProps) {
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'live_matches' }, payload => {
                ...
                setCurrentMatch((prev: any) => {
                    ...
                    if (updated.game_state?.rematch_request?.accepted && updated.game_state?.rematch_new_room_id) {
                        handleJoinRematchRoom(updated.game_state.rematch_new_room_id);
                    }
                    return updated;
                });
            }).subscribe();
    ...
    const handleCreateMatch = async () => {
        ...
        else if (selectedGameType === 'beast')       initialState = {};
    ...
    const handleJoinMatch = async () => {
        ...
        const newStatus = (match.game_type === 'stopthebus' || match.game_type === 'hangman' || match.game_type === 'bottlematch' || match.game_type === 'puzzle' || match.game_type === 'memory' || match.game_type === 'beast') ? 'waiting' : 'playing';
    ...
    // ── Rematch: نفس اللاعبين، نفس اللعبة، غرفة جديدة ──────────────────────
    const handleRematch = async () => {
        if (!currentMatch) return;
        setLoading(true);
        const gameType = currentMatch.game_type;
        const oldPlayers: any[] = currentMatch.players ?? [];

        // أنشئ غرفة جديدة بنفس اللاعبين
        let initialState: any = {};
        if (gameType === 'stopthebus') initialState = { letter: '', startedAt: 0, records: [], voteRound: 1 };

        const myPlayer = oldPlayers.find((p: any) => p.id === employee.employee_id);
        const { data, error } = await supabase.from('live_matches').insert({
            game_type: gameType,
            status: 'waiting',
            players: myPlayer ? [myPlayer] : [],
            game_state: initialState,
            created_by: employee.employee_id,
        }).select().single();

        if (error || !data) { setLoading(false); return toast.error('خطأ في إنشاء غرفة جديدة'); }

        // أرسل الرابط تلقائياً عبر الـ toast مع نسخه
        copyRoomLink(data.id);
        toast.success('غرفة جديدة! تم نسخ الرابط — أرسله لزميلك 🔗', { duration: 4000, icon: '🔄' });
        ...
        <div className="bg-gray-50 min-h-full flex flex-col relative font-sans text-right" dir="rtl">
            ...
            {view === 'playing' && currentMatch && (
                <div className="flex-1 flex flex-col">
                    ...
                    {currentMatch.game_type !== 'stopthebus' && currentMatch.game_type !== 'hangman' && currentMatch.game_type !== 'bottlematch' && currentMatch.game_type !== 'puzzle' && currentMatch.game_type !== 'memory' && currentMatch.game_type !== 'beast' && (
                    ...
                    <div className={`flex-1 flex flex-col ${
                        ...
                        {currentMatch.status === 'waiting' && currentMatch.game_type !== 'stopthebus' && currentMatch.game_type !== 'hangman' && currentMatch.game_type !== 'bottlematch' && currentMatch.game_type !== 'puzzle' && currentMatch.game_type !== 'memory' && currentMatch.game_type !== 'beast' && (
                            <div className="text-center">
                                ...
                                {/* Share button — visible for all players in waiting */}
                                <div className="mt-6 flex items-center gap-3 justify-center flex-wrap">
                                    <button onClick={() => {
                                        copyRoomLink(currentMatch.id);
                                        toast.success('تم نسخ رابط الغرفة! أرسله لزميلك 🔗', { icon: '📋', duration: 3000 });
                                    }} className="bg-green-500 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2 hover:bg-green-600 active:scale-95 transition-all text-sm shadow-lg">
                                        <Share2 size={16}/> شارك الرابط
                                    </button>
                                    {currentMatch.created_by === employee.employee_id && (
                                        <button onClick={() => handleDeleteMatch(currentMatch.id)}
                                            className="bg-red-50 text-red-500 border border-red-200 px-5 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-red-100 active:scale-95 transition-all text-sm shadow-sm">
                        ...
                        {/* ── REMATCH BAR — يظهر بعد انتهاء أي لعبة ── */}
                        {currentMatch.status === 'finished' && (
                            <div className="px-3 pb-3 pt-1">
                                <div className="flex gap-2">
                                    <button onClick={handleRematch} disabled={loading}
                                        className="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 rounded-2xl font-black text-sm shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-60">
                                        {loading ? <Loader2 className="w-4 h-4 animate-spin"/> : <>🔄 العب مرة ثانية</>}
                                    </button>
                                    <button onClick={exitMatch}
                                        className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-2xl font-black text-sm hover:bg-gray-200 active:scale-95 transition-all flex items-center justify-center gap-2">
                                        🏠 الصالة
                                    </button>
                                </div>
                            </div>
