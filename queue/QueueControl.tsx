...
import { 
    ...
    PlaySquare, Siren, MessageSquare, Send, X
} from 'lucide-react';
...
export default function QueueControl({ isAdmin = false }: { isAdmin?: boolean }) {
    ...
    // تسجيل الدخول يتطلب اختيار عيادة، حتى لو كان مديراً
    ...
    // الاستماع للرسائل والإنذارات الواردة
    useEffect(() => {
        const sub = supabase.channel('clinic_alerts_control')
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'q_alerts' }, (payload) => {
                ...
                // استقبال تحويل أو رسالة لنفس العيادة
                if (alert.clinic_id === selectedClinic.id) {
                    if (alert.type === 'transfer') {
                        ...
                        toast(`رسالة واردة: ${alert.message}`, { duration: 10000, icon: '📩' });
                    }
                }
            }).subscribe();
    ...
    const triggerAlert = async (type: string, message: string, targetClinicId?: string) => {
        ...
        await supabase.from('q_alerts').insert({
            ...
            clinic_id: targetClinic?.id || null,
            message: message,
            type: type
        });
            ...
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 text-right" dir="rtl">
                <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border border-gray-100">
                    ...
                    <div className="space-y-4">
                        ...
                        {!isAdmin && (
                            <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full p-3 rounded-xl border bg-gray-50 font-bold text-center text-2xl tracking-widest" placeholder="الرقم السري للعيادة" />
                        )}
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 md:p-6 text-right pb-20 relative" dir="rtl">
            <div className="max-w-lg mx-auto space-y-4">
                
                {/* Header (Allows Admin to Switch Clinics on the fly) */}
                ...
                <div className="grid grid-cols-3 gap-2">
                    <button onClick={() => triggerAlert('call', selectedClinic.current_number)} className="bg-white p-3 rounded-2xl shadow-sm border font-bold text-gray-700 flex flex-col items-center gap-2 text-xs hover:bg-gray-50">
                        <BellRing className="w-6 h-6 text-amber-500"/> تكرار النداء
                    </button>
                    <button onClick={() => setActiveModal('transfer')} className="bg-white p-3 rounded-2xl shadow-sm border font-bold text-gray-700 flex flex-col items-center gap-2 text-xs hover:bg-gray-50">
                        <ArrowLeftRight className="w-6 h-6 text-blue-500"/> تحويل مريض
                    </button>
                    <button onClick={() => setActiveModal('name')} className="bg-white p-3 rounded-2xl shadow-sm border font-bold text-gray-700 flex flex-col items-center gap-2 text-xs hover:bg-gray-50">
                        <Megaphone className="w-6 h-6 text-pink-500"/> إذاعة اسم
                    </button>
                    <button onClick={() => setActiveModal('record')} className="bg-white p-3 rounded-2xl shadow-sm border font-bold text-gray-700 flex flex-col items-center gap-2 text-xs hover:bg-gray-50">
                        <PlaySquare className="w-6 h-6 text-purple-500"/> تشغيل ريكورد
                    </button>
                    <button onClick={() => setActiveModal('alarm')} className="bg-white p-3 rounded-2xl shadow-sm border font-bold text-gray-700 flex flex-col items-center gap-2 text-xs hover:bg-gray-50">
                        <Siren className="w-6 h-6 text-red-500"/> إنذار
                    </button>
                    <button onClick={() => setActiveModal('message')} className="bg-white p-3 rounded-2xl shadow-sm border font-bold text-gray-700 flex flex-col items-center gap-2 text-xs hover:bg-gray-50">
                        <MessageSquare className="w-6 h-6 text-emerald-500"/> رسالة
                    </button>
                    
                    <button onClick={() => { if(confirm('تأكيد التصفير؟')) updateNumber(0); }} className="col-span-1 bg-white p-3 rounded-2xl shadow-sm border font-bold text-gray-700 flex flex-col items-center gap-2 text-xs hover:bg-gray-50">
                        <RotateCcw className="w-6 h-6 text-gray-400"/> تصفير
                    </button>
                    ...
                    <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl relative">
                        ...
                        {activeModal === 'transfer' && (
                            <div className="space-y-4 pt-4">
                                ...
                                <p className="text-xs text-gray-500">سيتم نداء العميل رقم ({selectedClinic.current_number}) للتوجه للعيادة التالية:</p>
