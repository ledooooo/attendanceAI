        ...
        <div className="space-y-6 animate-in fade-in pb-20">
            ...
            <div className="flex bg-white p-1 rounded-2xl shadow-sm border border-gray-100 w-fit">
                <button 
                    ...
                    className={`px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'new' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
                >
                    <Send className="w-4 h-4"/> إرسال جديد
                </button>
                <button 
                    onClick={() => setActiveTab('history')} 
                    className={`px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'history' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
                >
                ...
                <div className="bg-white p-6 rounded-[30px] border shadow-sm space-y-6">
                    ...
                    <div className="space-y-3 border-b pb-6">
                        <label className="text-sm font-black text-gray-700 flex items-center gap-2">
                            <Users className="w-4 h-4 text-indigo-500"/> لمن توجه هذا التكليف؟
                        </label>
                        ...
                        {targetType === 'individual' && (
                            <>
                                ...
                                <div>
                                    ...
                                    <select 
                                        className="w-full p-3 bg-white border-2 border-indigo-100 focus:border-indigo-500 rounded-xl text-sm outline-none font-bold"
                                        value={selectedEmp} onChange={e => setSelectedEmp(e.target.value)}
                                    >
                                        <option value="">اختر الموظف...</option>
                                        {filteredEmployees.map(e => (
                                            <option key={e.employee_id} value={e.employee_id}>
                                                {e.name} {e.status !== 'نشط' ? '(غير نشط)' : ''}
                                            </option>
                                        ))}
                                    </select>
                                </div>
                    ...
                    <div className="space-y-4 pt-4 border-t">
                        <div>
                            ...
                            <input 
                                type="text" placeholder="مثال: تعليمات بخصوص المرور الصباحي" 
                                className="w-full p-3 border rounded-xl bg-gray-50 focus:bg-white focus:border-indigo-500 outline-none font-bold text-sm transition-all"
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                className="w-full p-3 border rounded-xl bg-gray-50 focus:bg-white focus:border-indigo-500 outline-none text-sm h-24 resize-none transition-all"
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-2">
                        ...
                        <button 
                            ...
                            className="w-full md:w-auto bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 disabled:opacity-50"
                        >
                            {sendTaskMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin"/> : <Send className="w-5 h-5 rtl:rotate-180"/>} 
                            إرسال التكليف
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[30px] border shadow-sm overflow-hidden flex flex-col h-[600px]">
                    <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
                        <h3 className="font-bold text-gray-700 flex items-center gap-2">
                            <CheckSquare className="w-5 h-5 text-indigo-600"/> سجل التكليفات الصادرة
                        </h3>
                        <button 
                            onClick={() => refetch()} 
                            disabled={isRefetching}
                            className="p-2 bg-white border rounded-lg text-gray-600 hover:text-indigo-600 hover:border-indigo-200 transition-all"
                        >
                    ...
                    <div className="overflow-y-auto custom-scrollbar flex-1 p-2">
                        {loadingHistory ? (
                            ...
                            <div className="text-center py-20 text-gray-400 font-bold border-2 border-dashed border-gray-100 rounded-3xl m-4">
                                لا توجد تكليفات سابقة
                            </div>
                        ) : (
                            <table className="w-full text-sm text-right">
                                <thead className="bg-gray-50 text-gray-500 font-bold sticky top-0 z-10">
                                    <tr>
                                        ...
                                        <th className="p-4">الموظف / القسم</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {tasks.map((task: any) => (
                                        <tr key={task.id} className="hover:bg-indigo-50/30 transition-colors group">
                                            ...
                                            <td className="p-4 text-xs text-gray-600 max-w-[200px]">
                                                {task.response_note ? (
