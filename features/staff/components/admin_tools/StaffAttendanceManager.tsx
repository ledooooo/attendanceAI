...
import { 
    ...
    ArrowUpDown, PlusCircle, Save, X, UserCheck, FilePlus, Clock, Moon, Sun, XCircle, FileSpreadsheet
} from 'lucide-react';
...
export default function ReportsTab() {
    ...
    const processedData = useMemo(() => {
        ...
        let data = employees.map(emp => {
            ...
            if (printOverrides[emp.employee_id]) {
                const overrideVal = printOverrides[emp.employee_id];
                displayIn = overrideVal;
                displayOut = '';
                // "غياب" للطباعة فقط، لكن إحصائياً يظل "غير متواجد"
                if (overrideVal === 'غياب') statsStatus = 'غياب_طباعة'; 
                else if (overrideVal === 'مسائي' || overrideVal === 'مبيت') statsStatus = overrideVal;
    ...
    const stats = useMemo(() => {
        ...
        const absent = processedData.filter(d => d.statsStatus === 'غير متواجد' || d.statsStatus === 'غياب_طباعة').length;
        ...
        processedData.forEach((d: any) => {
            if (!bySpecialty[d.specialty]) bySpecialty[d.specialty] = { 
                total: 0, present: 0, absent: 0, leave: 0, evening: 0, overnight: 0, partTimeOff: 0 
            };
            
            bySpecialty[d.specialty].total++;
            
            if (d.statsStatus === 'متواجد') bySpecialty[d.specialty].present++;
            else if (d.statsStatus === 'غير متواجد' || d.statsStatus === 'غياب_طباعة') bySpecialty[d.specialty].absent++;
            else if (d.statsStatus === 'إجازة') bySpecialty[d.specialty].leave++;
            else if (d.statsStatus === 'مسائي') bySpecialty[d.specialty].evening++;
            else if (d.statsStatus === 'مبيت') bySpecialty[d.specialty].overnight++;
            else if (d.statsStatus === 'جزء وقت') bySpecialty[d.specialty].partTimeOff++;
        });
    ...
    // --- Handlers ---
    const handleQuickAction = (action: string, empId: string) => {
        if (action === 'attendance') {
            ...
            setPrintOverrides(prev => ({ ...prev, [empId]: action }));
            toast(`تم التعيين: ${action}`, { icon: '📝' });
        }
    };
            ...
            <div ref={componentRef} className="bg-white p-4 rounded-[30px] shadow-sm min-h-[800px] print:p-2 print:shadow-none print:w-full" dir="rtl">
                ...
                <div className="hidden print:block text-center border-b border-black pb-1 mb-2">
                    <p className="text-[12px] font-bold font-mono text-black leading-tight">
                        ...
                        {activeReport !== 'monthly' && ` تحريراً في: ${new Date(date).toLocaleDateString('ar-EG')}`}
                    </p>
                </div>
                                ...
                                <>
                                    ...
                                    {openMenuId === row.id && (
                                        <div className="absolute left-0 top-6 w-40 bg-white shadow-xl rounded-xl border z-50 overflow-hidden animate-in zoom-in-95">
                                            ...
                                            {/* ✅ خيار غياب للطباعة فقط */}
                                            <button onClick={() => { onQuickAction('غياب', row.employee_id); setOpenMenuId(null); }} className="w-full text-right px-4 py-2 hover:bg-gray-50 text-xs font-bold flex items-center gap-2 text-red-700 border-t"><XCircle className="w-3 h-3"/> غياب (طباعة)</button>
    ...
    <table className="w-full text-sm text-right border-collapse">
        <thead className="bg-gray-100 font-bold border-b border-black">
            <tr>
                ...
                <th className="p-1 border border-gray-400">الاسم</th>
                <th className="p-1 border border-gray-400">الوظيفة</th>
                <th className="p-1 border border-gray-400">الرقم القومي</th>
                <th className="p-1 border border-gray-400">التليفون</th>
                <th className="p-1 border border-gray-400">المهام الادارية</th>
            </tr>
        </thead>
        <tbody>
            {data.map((emp, idx) => (
                <tr key={emp.id} className="border-b border-gray-300">
                    ...
                    <td className="p-1 border border-gray-300 text-center font-mono">{emp.phone}</td>
...
const SpecialtiesTable = ({stats}: {stats:any}) => (
    <table className="w-full text-[11px] text-right border-collapse max-w-4xl mx-auto">
        <thead className="bg-gray-800 text-white font-bold">
            <tr>
                <th className="p-1 border border-gray-600">التخصص</th>
                <th className="p-1 border border-gray-600 text-center">إجمالي</th>
                ...
                <th className="p-1 border border-gray-600 text-center">جزء وقت راحة</th>
                <th className="p-1 border border-gray-600 text-center">% الحضور</th>
                <th className="p-1 border border-gray-600 text-center">% الإجازات</th>
            </tr>
        </thead>
        <tbody>
            {Object.entries(stats.bySpecialty).map(([spec, s]: any) => {
                const totalEffective = s.total - s.partTimeOff; // الإجمالي الفعلي (مطروح منه الراحات)
                const presenceRate = totalEffective > 0 ? Math.round(((s.present + s.evening + s.overnight) / (totalEffective - s.leave)) * 100) : 0;
                const leaveRate = s.total > 0 ? Math.round((s.leave / s.total) * 100) : 0;
                ...
                return (
                    <tr key={spec} className="border-b border-gray-300">
                        ...
                        <td className="p-1 border border-gray-300 text-center font-mono">{presenceRate}%</td>
