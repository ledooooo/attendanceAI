...
import { 
    Clock, Calendar, AlertTriangle, Briefcase, Activity, FileText, Calculator, Coffee, UserX
} from 'lucide-react';
...
const getFiscalYearRange = () => {
    ...
    // ضبط الوقت ليكون في بداية اليوم ونهايته تماماً لتجنب إزاحة التوقيت
...
export default function StaffStats({ attendance, evals, requests, month, employee }: Props) {
    ...
    const leavesStats = useMemo(() => {
        const stats = {
            ...
            rest: { used: 0, earned: 0, label: 'بدل راحة' }
        };
        ...
        requests.forEach(req => {
            ...
            if (reqStatus === 'مقبول' && (typeKey === 'annual' || typeKey === 'casual' || typeKey === 'rest')) {
                ...
                if (reqStart >= startDate && reqStart <= endDate) {
                    ...
                    if (typeKey === 'rest') stats.rest.used += count;
                    else stats[typeKey].used += count;
                }
            }
        });
        return stats;
    ...
    // 2. إحصائيات الحضور والغياب (شهري وأسبوعي)
    const attendanceStats = useMemo(() => {
        let totalMonthlyHours = 0;
        let lateCount = 0;
        let restEarnedDays = 0; // استحقاق بدل راحة (> 9 ساعات)
        let presentDays = 0;

        const officialStart = employee?.start_time || "08:30";
        const [offH, offM] = officialStart.split(':').map(Number);

        // تصفية بيانات الشهر الحالي
        ...
        monthlyRecords.forEach(att => {
            ...
            if (times.length >= 2) {
                ...
                if (hours > 9) restEarnedDays++; // يستحق يوم بدل راحة
        ...
        // حساب الغياب التقريبي (أيام العمل المفترضة 26 مطروحاً منها الحضور والإجازات)
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-700 text-right" dir="rtl">
            ...
            {/* بطاقة الأرصدة الكبرى */}
            <div className="bg-gradient-to-br from-slate-900 to-indigo-900 p-8 rounded-[40px] shadow-2xl relative overflow-hidden text-white">
                <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white/10 p-6 rounded-[30px] border border-white/10">
                            ...
                            <div className="flex justify-between items-end">
                                <div>
                                    <span className="text-4xl font-black">{attendanceStats.restEarnedDays - leavesStats.rest.used}</span>
                                    <p className="text-[10px] opacity-60">رصيد متاح</p>
                                </div>
                                <div className="text-left text-xs">
                                    <p className="text-emerald-400">المستحق: {attendanceStats.restEarnedDays}</p>
                                    <p className="text-red-400">المطلب: {leavesStats.rest.used}</p>
                                </div>
                            </div>
                        </div>
                    </div>
