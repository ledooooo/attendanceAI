...
import { User, Hash, ShieldCheck, Briefcase, CalendarDays, Phone } from 'lucide-react';
...
export default function StaffProfile({ employee }: { employee: Employee }) {
    return (
        <div className="space-y-8 animate-in slide-in-from-bottom duration-500">
            <h3 className="text-2xl font-black flex items-center gap-3 text-gray-800 border-b pb-4"><User className="text-emerald-600 w-7 h-7" /> الملف الشخصي</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <ProfileItem label="التخصص" value={employee.specialty} icon={Briefcase} />
                <ProfileItem label="تاريخ التعيين" value={employee.join_date} icon={CalendarDays} />
                <ProfileItem label="الهاتف" value={employee.phone} icon={Phone} />
                <ProfileItem label="البريد الإلكتروني" value={employee.email} icon={User} />
            </div>
        </div>
    );
}