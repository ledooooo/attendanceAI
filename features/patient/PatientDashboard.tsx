...
// ✅ استيراد معالج الملف الطبي الذي أنشأناه
...
export default function PatientDashboard() {
  ...
  // 🌟 حالات الفحص (هل لديه ملف طبي أم لا؟)
  ...
  useEffect(() => {
    const checkPatientProfile = async () => {
      ...
      try {
        ...
        if (data) {
          setHasProfile(true); // لديه ملف
        } else {
          setHasProfile(false); // ليس لديه ملف (مريض جديد)
        }
      } catch (err) {
  ...
  useEffect(() => {
    // نجلب المقالات فقط إذا كان لديه ملف لتقليل الطلبات على السيرفر
  ...
  const menuItems = [
    ...
    { id: 'medical_file', label: 'الملف الطبي', icon: FileText },
    { id: 'family', label: 'أسرتي', icon: Users },
    { id: 'logs', label: 'سجلاتي الصحية', icon: Activity },
  ...
  const bottomNavItems = [
    ...
    { id: 'appointments', label: 'مواعيدي', icon: Calendar },
    { id: 'logs', label: 'سجلاتي', icon: Activity },
  ...
  // إذا لم يكن يملك ملفاً، نعرض له المعالج. عند الانتهاء (onComplete) نحول الحالة إلى true.
  if (hasProfile === false) {
    return <MedicalProfileWizard onComplete={() => setHasProfile(true)} />;
  }

  // ─── واجهة المريض الرئيسية (تظهر إذا كان hasProfile === true) ───
  return (
