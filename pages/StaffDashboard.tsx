    ...
    if (selectedTemplate) {
        return (
            <div className="animate-in fade-in duration-300">
                {/* زر العودة وزر الطباعة - يختفيان عند الطباعة */}
                ...
                <div className="print-paper mx-auto bg-white p-12 max-w-[210mm] min-h-[297mm] relative text-black print:w-full print:max-w-none print:p-0">
                    ...
                    <div className="flex justify-between items-start border-b-4 border-black pb-6 mb-12">
                        ...
                        <div className="w-24 h-24 flex items-center justify-center opacity-80">
                             {/* شعار وزارة الصحة - رابط صورة عام */}
                    ...
                    <div className="text-center my-16">
                        <h1 className="text-3xl font-black underline decoration-2 underline-offset-8 border-2 border-black inline-block px-8 py-2 rounded-lg">{selectedTemplate.title}</h1>
                    </div>

                    {/* محتوى النموذج الديناميكي */}
                    <div className="text-2xl leading-[3] text-justify font-medium px-4 whitespace-pre-line">
                        {/* هنا يتم استدعاء الدالة الموجودة في المصفوفة وتمرير بيانات الموظف لها */}
                    ...
                    <div className="absolute bottom-10 left-0 right-0 text-center text-xs text-gray-500 border-t pt-2 w-full">
                        تم استخراج هذا المستند إلكترونياً من نظام إدارة المركز الطبي بتاريخ {new Date().toLocaleDateString('ar-EG')}
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-8 animate-in slide-in-from-bottom duration-500 no-print">
            <h3 className="text-2xl font-black flex items-center gap-3 text-gray-800 border-b pb-4">
                <FileCheck className="text-emerald-600 w-7 h-7" /> نماذج رسمية جاهزة للطباعة
            </h3>
            <p className="text-gray-500 font-bold">اختر النموذج الذي تريد طباعته وسيتم ملء بياناتك تلقائياً:</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {OFFICIAL_TEMPLATES.map((tmpl) => (
                    ...
                    >
                        ...
                        <ArrowRight className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-300 group-hover:text-emerald-500 -translate-x-4 group-hover:translate-x-0 transition-all opacity-0 group-hover:opacity-100" />
                    </button>
                ))}
            </div>
        </div>
